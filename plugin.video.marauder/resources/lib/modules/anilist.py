...
'''
10.20.2019: Updated Anilist.CO API Key details
'''
...
def _getAniList(url):
    try:
        ...
        return client.request(
            url,
            headers={'Authorization': '%s %s' % cache.get(_getToken, 1),
...
def _getToken():
    result = urllib.urlencode(
        {'grant_type': 'client_credentials', 'client_id': '2732',
         'client_secret': 'yvY3prdvzhb2hS5xfSdZDdtfBFQ2J2uEAKC7sSBZ'})
...
def getAlternativTitle(title):
    try:
        ...
        r = [(i.get('title_romaji'), i.get('synonyms', []))
             for i in utils.json_loads_as_str(r) if cleantitle.get(i.get('title_english', '')) == t]
