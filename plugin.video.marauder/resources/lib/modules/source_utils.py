...
def get_release_quality(release_name, release_link=None):
    ...
    try:
        ...
        # elif '.WEBRip.' in fmt: quality = '720p'
            ...
            quality = 'SCR'
        elif any(i in [' camrip ', ' tsrip ', ' hdcam ', ' hd cam ', ' cam rip ', ' hdts ', ' dvdcam ', ' dvdts ', ' cam ', ' telesync ', ' ts '] for i in fmt):
            quality = 'CAM'
            ...
            if release_link:
                ...
                if ' 4k ' in release_link:
                    ...
                    if any(i in [' dvdscr ', ' r5 ', ' r6 '] for i in release_link):
                        quality = 'SCR'
                    elif any(i in [' camrip ', ' tsrip ', ' hdcam ', ' hd cam ', ' cam rip ', ' hdts ', ' dvdcam ', ' dvdts ', ' cam ', ' telesync ', ' ts '] for i in release_link):
                        quality = 'CAM'
                    else:
                        quality = 'SD'
            else:
...
def is_host_valid(url, domains):
    try:
        if any(x in url.lower() for x in ['.rar.', '.zip.', '.iso.']) or any(
                url.lower().endswith(x) for x in ['.rar', '.zip']):
                    raise Exception()
